...
<head>
  <meta charset="UTF-8">
  <title><fmt:message key="conge.nouveau.titre" /></title>
...
<div class="container-fluid">
  <div class="row">
    ...
    <main id="mainContent" class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4 animate__animated animate__fadeIn">
      <h3 class="mb-4">📝 <fmt:message key="conge.nouveau.titre" /></h3>
      ...
      <form action="${pageContext.request.contextPath}/employe/conges/demander" method="post" class="row g-3">
        <div class="col-md-6">
          <label for="dateDebut" class="form-label"><fmt:message key="conge.date.debut" /></label>
          <input type="date" class="form-control" name="dateDebut" id="dateDebut" required>
        </div>
        <div class="col-md-6">
          <label for="dateFin" class="form-label"><fmt:message key="conge.date.fin" /></label>
          <input type="date" class="form-control" name="dateFin" id="dateFin" required>
        </div>
        <div class="col-12">
          <label for="motif" class="form-label"><fmt:message key="conge.motif" /></label>
          <textarea class="form-control" name="motif" rows="3" id="motif" required></textarea>
        </div>
        <div class="col-12">
          <button type="submit" class="btn btn-primary">
            <i class="bi bi-check-circle me-1"></i> <fmt:message key="conge.bouton.envoyer" />
          </button>
        </div>
      </form>
    </main>
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector("form").addEventListener("submit", function (e) {
      ...
      if (dateDebut < now || dateFin < now) {
        e.preventDefault();
        alert("❌ <fmt:message key='conge.erreur.date.passee' />");
      } else if (dateDebut > dateFin) {
        e.preventDefault();
        alert("❌ <fmt:message key='conge.erreur.date.invalide' />");
      }
    });
  });
</script>
